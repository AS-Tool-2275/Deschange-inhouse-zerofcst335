...
- `SI = 0` sẽ được hiểu là **hard lock**: warehouse đó giữ nguyên quantity gốc.
- Nhóm warehouse có `Average of SS Wk3 = 0` sẽ có thêm fallback equalization pass để cân bằng SI After trong cùng nhóm.
...
st.write("Để trống nếu không có kho ưu tiên. `Value`: nhập 50 hoặc 0.5 đều được hiểu là 50%; nhập 1 được hiểu là 100%. `SI = 0` sẽ hard lock warehouse đó.")
