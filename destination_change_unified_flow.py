...
def build_rows(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> Tuple[List[dict], int]:
    ...
    for _, r in group.iterrows():
        ...
        if rule is not None:
            ...
            row["hard_locked"] = rule.mode == "SI" and float(rule.value or 0.0) <= 0.0
        rows.append(row)
...
def choose_lowest_ss_recipient(rows: List[dict], candidate_indices: List[int]) -> Optional[int]:
    ...
    for idx in candidate_indices:
        ...
        if row.get("hard_locked"):
            continue
        after_si = current_si_after(row)
        ratio = safe_ss_ratio(after_si, row["ss_target"])
        candidates.append((ratio, after_si, row["whse"], idx))
    if not candidates:
        return None
...
def allocate_item(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> pd.DataFrame:
    ...
    for row in rows:
        if row.get("hard_locked"):
            ...
            hard_locked_total += row["orig_f"]

    remaining = total_f - hard_locked_total
    if remaining < 0:
        raise ValueError(f"Item {rows[0]['item']}: hard lock exceeds total firm quantity.")

    priority_indices = [i for i, r in enumerate(rows) if r["priority_rule_mode"] and not r["hard_locked"]]
    non_priority_indices = [i for i, r in enumerate(rows) if not r["priority_rule_mode"] and not r["hard_locked"]]
    ...
    if remaining > 0 and allocation_pool:
        ...
        for idx in allocation_pool:
            ...
            after_si = current_si_after(row)
            ratio = safe_ss_ratio(after_si, row["ss_target"])
            heapq.heappush(heap, (ratio, after_si, row["whse"], idx))
    ...
    out["Hard Locked"] = [r["hard_locked"] for r in rows]
...
def apply_zero_ss_equalization(detail: pd.DataFrame) -> pd.DataFrame:
    """
    Fallback pass for items with 2+ zero-SS warehouses.
    Equalizes SI After within the zero-SS group while preserving total firm quantity.
    Hard-locked rows are excluded.
    """
    if detail is None or detail.empty:
        return detail

    df = detail.copy()
    if "Hard Locked" not in df.columns:
        df["Hard Locked"] = False

    numeric_cols = [
        "Average of SS Wk3", "F Wk3 Original", "F Wk3 After Destination Change",
        "Net Destination Change", "Current SI After", "SS % After", "Current SI",
    ]
    ...
    for item, grp in df.groupby("Item", sort=False):
        zero_idx = [
            idx for idx in grp.index
            if float(df.at[idx, "Average of SS Wk3"]) <= 0
            and not bool(df.at[idx, "Hard Locked"])
        ]
        if len(zero_idx) < 2:
            continue
        ...
        while guard < 1000:
            guard += 1
            curr = pd.to_numeric(df.loc[zero_idx, "Current SI After"], errors="coerce").fillna(0.0)
            donor = curr.idxmax()
            recipient = curr.idxmin()
            if donor == recipient:
                break
        ...
        # Recompute SS% After for the zero-SS subset after equalization
        for idx in zero_idx:
            ss_target = float(df.at[idx, "Average of SS Wk3"])
            df.at[idx, "SS % After"] = safe_ss_ratio(float(df.at[idx, "Current SI After"]), ss_target)
...
def build_detail_output(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    preferred = [
        ...
        "Priority Rule Mode", "Priority Rule Value", "Priority Target F After", "Hard Locked",
    ]
...
def run_optimizer(optimizer_input: pd.DataFrame, priority_rules: Optional[Dict[str, PriorityRule]] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Second pass: if an item has 2+ zero-SS warehouses, rebalance by SI After.
    detail_full = apply_zero_ss_equalization(detail_full)

    # Keep subvendor suggestion after the main-vendor result.
